...
new class extends Component {
    ...
    {
        return ActivityLog::where('deal_id', $this->dealId)->orderBy('created_at', 'desc')->get();
    }
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-100">
        <form wire:submit.prevent="submit">
            <div class="flex items-center justify-between gap-4 mb-4">
                ...
                <input wire:model="activityName" type="text"
                    class="block w-full pl-4 pr-3 py-2 text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 transition"
            ...
            <textarea wire:model="message"
                ...
                placeholder="{{ $type === 'email' ? 'Email body (leave blank to use template)' : 'Description' }}"></textarea>
            ...
            @forelse ($this->activities as $activity)
                <div class="border-b border-gray-200 py-3 dark:border-slate-700">
                    <div class="flex items-start gap-3">
                        ...
                        @if (in_array($activity->type, ['tasks', 'todo']))
                            <button wire:click="toggleStatus({{ $activity->id }})" wire:loading.attr="disabled"
                        ...
                        @else
                            <div
                                class="mt-0.5 flex-shrink-0 h-5 w-5 rounded-full bg-indigo-100 dark:bg-indigo-900 flex items-center justify-center">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <span
                                    ...
                                    @else bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300 @endif">
