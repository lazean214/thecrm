...
new class extends Component {
    ...
    public function setView(string $view): void
    {
        ...
        // FIX: Set loading flags appropriately for both views
        if ($view === 'table') {
    ...
    public function updateStage(int $dealId, string $newStage): void
    {
        ...
        // FIX: Check stage permissions properly
        if (!$user->canMoveToStage($newStage)) {
            $this->showPermissionModal($deal->name, $newStage);
            return;
        }
    ...
    {
        $this->confirmMessage = count($this->selectedDeals) . ' deal(s) will be deleted. Cannot be undone.';
        $this->showBatchModal = false;
    ...
    private function buildBaseQuery(): \Illuminate\Database\Eloquent\Builder
    {
        ...
        // Auth - visible to user's deals or whole pipeline if not sales
        if (!auth()->check()) {
...
<div class="space-y-6 w-full mx-auto p-4 sm:p-6 lg:p-8 antialiased text-slate-900 dark:text-slate-100"
    ...
    {{-- Kanban View --}}
    @if ($view === 'kanban')
        <div wire:key="kanban-board" class="transition-opacity duration-200" @class(['opacity-50 pointer-events-none' => $kanbanLoading])>

            {{-- Loading skeleton --}}
            @if ($kanbanLoading)
                <div class="flex gap-3 overflow-x-auto pb-4 px-2">
                    @foreach (range(1, 5) as $i)
                        <div
                            class="flex-shrink-0 w-[280px] md:w-[320px] h-[650px] bg-slate-200 dark:bg-slate-800 rounded-2xl animate-pulse">
                        </div>
                    @endforeach
                </div>
            @else
    ...
    {{-- Table View --}}
    @if ($view === 'table')
        <div wire:key="table-view-{{ $currentPage }}" @class(['opacity-50 pointer-events-none' => $tableLoading])
